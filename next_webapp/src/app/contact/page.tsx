...
// TypeScript interfaces for form fields and their validation
interface FormField {
  name: string;
  spanName: string;
  type: string;
...
const Contact = () => {
  const formFields = [
    ...
    },
    {
      ...
      required: true,
      validator: (phone: string) => /^\d{10,}$/.test(phone.replace(/\D/g, '')),  // Adjust regex as per your phone format needs
    },
    {
      name: "message",
      spanName: "Message",
...
//       <main className="contactBody font-light text-xs leading-7 flex flex-col justify-between mt-12 items-start p-12">
//         <div className="formContent w-full">
//           <form id="contact" onSubmit={handleSubmit} className="m-8">
//             {formFields.map((field) => (
//               <fieldset key={field.name} className="border-0 m-0 mb-2.5 min-w-full p-0 w-full text-gray-700">
//                 <span className="namaSpan text-black">{field.spanName}</span>
//                 {field.type === "textarea" ? (
...
//             ))}
//             <div className="flex justify-center items-center">
//               <button type="submit" className="bg-green-800 text-white font-semibold text-lg py-1 px-6 rounded hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-700 focus:ring-opacity-50">
//                 Submit
//               </button>
//             </div>
//           </form>
