...
export async function POST(request: Request) {
   ...
   } else if (searchMode === SEARCH_MODE.TERM) {
      // Filter by term folder (e.g., "T1 2024", "T2 2024")
      const filteredTermFolders = termFolders.filter((termFolder) =>
         termFolder.toLowerCase().includes(searchTerm.toLowerCase())
      );

      console.log("🚀 ~ POST ~ filteredTermFolders:", filteredTermFolders);

      filteredStudies = caseStudies.filter((caseStudy) =>
         filteredTermFolders.some((folder) =>
            caseStudy.filename?.toLowerCase().includes(folder.toLowerCase())
         )
      );
   }
