...
export async function POST(request: Request) {
  ...
  // Define generic search terms that indicate a broad query
  const genericTerms = ["use cases", "what are use cases", "show me use cases", "list use cases"];
  const lowerSearchTerm = searchTerm.toLowerCase();

  // If the search term matches one of the generic terms, return a default subset
  if (genericTerms.includes(lowerSearchTerm)) {
    // Return, for example, the first 5 case studies (or adjust as needed)
    filteredStudies = caseStudies.slice(0, 5);
  } else if (searchMode === SEARCH_MODE.TITLE) {
    filteredStudies = caseStudies.filter((caseStudy) => {
      return caseStudy.name.toLowerCase().includes(lowerSearchTerm);
    });
