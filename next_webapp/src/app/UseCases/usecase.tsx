...
import { database, CaseStudy } from '../UseCases/database';
...
// This component is responsible for displaying the list of case studies
...
// This component is responsible for displaying the details of the selected case study
const CaseStudyPreview: React.FC<CaseStudyPreviewProps> = ({ caseStudy }) => (
    ...
    <div className="h-full bg-gray-200 rounded-lg overflow-hidden p-4">
      {caseStudy ? (
        ...
        <p>Select a case study to view details bit</p>
      )}
    </div>
  </div>
);
...
const CaseStudies: React.FC = () => {
  ...
  const uniqueGroups = Array.from(new Set(database.map(cs => cs.group)));

  // Filter the case studies based on the search term and selected category
  const filteredCaseStudies = database.filter(caseStudy =>
  ...
  return (
      <div className="container mx-auto px-1 py-2 pb-4">
        <h1 className="text-4xl font-bold mb-8 mt-4">Use Cases</h1>
