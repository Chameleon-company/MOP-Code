...
import { useDarkMode } from "@/context/DarkModeContext"; // Importing the darkmode context 

const Footer = () => {
  const t = useTranslations("common");
  const { darkMode } = useDarkMode(); // Uses the dark mode state from context

  return (
    <footer
      className={`${
        darkMode ? "bg-[#3A714E] text-white" : "bg-green-500 text-white"
      } py-8 mt-16 md:mt-8 transition-colors duration-300`}
    >
