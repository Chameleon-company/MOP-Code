...
model.load_state_dict(torch.load('metric_depth/models/depth_metric_vits.pth', map_location='cpu'))

model = model.to(DEVICE).eval()


raw_img = cv2.imread(args.img_path)
raw_depth = model.infer_image(raw_img) #depth infer
...
results = model_yolo.predict(source=args.img_path,save=False,device='cpu',conf=0.4)#yolo infer
