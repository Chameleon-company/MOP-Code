...
def _build_report_prompt(
    ...
    payload_block = (
        ...
        f"  Number Crack Regions      : {payload.num_crack_regions}\n"
        f"  Largest Crack Area Ratio  : {payload.largest_crack_area_ratio:.4f}\n"
        f"  Largest Crack Length      : {payload.largest_crack_est_length} px\n"
        f"  Severity                  : {payload.severity}\n"
        f"  Damage Level              : {payload.damage_level:.2f}"
    )
...
def generate_maintenance_report(
    ...
    if not documents:
        ...
        return MaintenanceReport(
                ...
                findings_summary=(
                    f"Crack(s) detected in image '{payload.image_id}'. "
                    f"Detected {payload.num_crack_regions} crack region(s). "
                    f"Largest region area ratio: {payload.largest_crack_area_ratio:.2f}. "
                    f"Largest crack length: {payload.largest_crack_est_length:.1f}px. "
                    f"Damage level: {payload.damage_level:.2f}. "
